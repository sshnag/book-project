...
<div class="container my-5">
   ...
   <div class="book-detail-card shadow rounded p-4 d-flex flex-wrap gap-4" style="background: #ffffff; border: 1px solid #eee;">
    ...
    <img src="{{ asset('storage/' . $book->cover_image) }}" alt="{{ $book->title }}" class="img-fluid rounded shadow" style="max-width: 250px; height: auto;  object-fit: contain;">

</div>
        <!-- Book Info -->
        <section class="info-wrapper flex-grow-1" style="min-width: 280px;">
            <h3 class="mb-3" style="font-family: 'Merriweather', serif; font-weight: 700; color: #151515;">{{ $book->title }}</h3>

            <p class="mb-1"><strong>Author: </strong> <span class="text-dark">{{ $book->author->name ?? 'N/A' }}</span></p>
            <p class="mb-1"><strong>Category: </strong> <span class="badge bg-info text-dark">{{ $book->category->name ?? 'N/A' }}</span></p>
            <p class="mb-3"><strong>Published: </strong> <i class="text-dark"> <time  datetime="{{ $book->published_at }}">{{ $book->published_at ? \Carbon\Carbon::parse($book->published_at)->format('F d, Y') : 'N/A' }}</time></i></p>
...
<div class="mt-4 d-flex gap-3 align-items-center">
    @auth
        <a href="{{ route('user.books.download', $book->id) }}" class="btn btn-outline-dark btn-back rounded-pill px-4"     {{ $book->title }}">
            <i class="fas fa-download me-2" aria-hidden="true"></i> Download PDF
        </a>
    @else
        <p class="text-danger mt-3 mb-0" role="alert">Please <a href="{{ route('login') }}" class="text-decoration-underline">log in</a> to download this book.</p>
    @endauth
    ...
    <section class="related-books mt-5">
        <h3 class="mb-4" style="font-family: 'Poppins', sans-serif; font-weight: 600; color: #34495e;">You might also like</h3>
